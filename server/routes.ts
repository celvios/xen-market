...
export async function registerRoutes(
  ...
  // Apply database indexes endpoint
  app.get("/api/admin/apply-indexes", async (req, res) => {
    try {
      ...
      res.json({ success: true, message: "Database indexes applied successfully" });
    } catch (error: any) {
      console.error("Index creation error:", error);
      res.status(500).json({ error: error.message || "Failed to apply indexes" });
    }
  });
